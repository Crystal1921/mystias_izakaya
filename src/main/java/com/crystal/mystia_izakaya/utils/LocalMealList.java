...
public class LocalMealList {
    ...
    private final List<Item> localMeals;
    ...
    private LocalMealList() {
        localMeals = List.of(ItemRegistry.Oedo_Boat_Feast.get(),
        ...
        for (FoodTagEnum foodTag : foodTags) {
            foodTypeMap.put(foodTag.name().toLowerCase(), foodTag);
        }
    }
    ...
    public List<CookedMealItem> getCookedMeals() {
        return localMeals.stream().map(item -> (CookedMealItem) item).toList();
    }
